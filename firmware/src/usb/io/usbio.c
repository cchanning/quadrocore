...
USBControlTransfer_t* USBBeginControlTransfer(USBEndpoint_t *usbEndpointP)
{
	...
	usbTransferP->usbEndpointP->usbEndpointInPipeP->cnt = 0;
	usbTransferP->usbEndpointP->usbEndpointOutPipeP->cnt = 0;
...
void USBProcessControlTransferOutput(USBControlTransfer_t *usbControlTransferP)
{
	ptr_t usbDataBufferOutP = usbControlTransferP->usbDataBufferOutP + usbControlTransferP->transmittedLength;
	...
	*/
	...
	{		
		...
		{
			memcpy(usbDataBufferOutP, (ptr_t)usbControlTransferP->usbEndpointP->usbEndpointOutPipeP->dataBufferP, usbControlTransferP->usbEndpointP->usbEndpointOutPipeP->cnt);
		}
